...
const NOVARENA_GAME_ID = "last-row-trouble";
let novarenaScoreSubmitted = false;
...
async function loadNovarenaGameContext() {
  if (!window.Novarena || typeof window.Novarena.getGameContext !== "function") {
    return null;
  }

  try {
    return await window.Novarena.getGameContext(NOVARENA_GAME_ID);
  } catch (error) {
    return null;
  }
}

function resetNovarenaRound() {
  novarenaScoreSubmitted = false;
}

function submitNovarenaScore(finalScore) {
  if (
    novarenaScoreSubmitted ||
    !window.Novarena ||
    typeof window.Novarena.submitScore !== "function"
  ) {
    return null;
  }
  ...
  novarenaScoreSubmitted = true;

  try {
    return Promise.resolve(window.Novarena.submitScore({
      game: NOVARENA_GAME_ID,
...
function endGame(title, reason) {
  ...
  submitNovarenaScore(game.score);
}
...
loadNovarenaGameContext().then((context) => {
