...
function syncPlatformScore() {
  if (!window.NovarenaClickerBridge || typeof window.NovarenaClickerBridge.submitScore !== "function") {
    return;
  }

  window.NovarenaClickerBridge.submitScore(
    Math.max(Math.floor(state.money), Math.floor(state.totalEarned))
  );
}
...
function resetGame() {
  ...
  if (window.NovarenaClickerBridge && typeof window.NovarenaClickerBridge.resetRound === "function") {
    window.NovarenaClickerBridge.resetRound();
  }
  updateStats();
}
...
function init() {
  ...
  setupTabs();
  bindEvents();
  if (window.NovarenaClickerBridge && typeof window.NovarenaClickerBridge.init === "function") {
    window.NovarenaClickerBridge.init();
  }
  updateStats();
  syncPlatformScore();
  startIncomeLoop();
